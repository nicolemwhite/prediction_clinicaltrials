...
web_history = NULL
studies = filter(decisions,final_decision =='include',overall_status %in% c('Active, not recruiting','Enrolling by invitation','Not yet recruiting','Recruiting')) %>% distinct(NCT) %>% pull()

f <- "data/clintrials_history_active_studies.rds"
if (file.exists(f)){
...
saveRDS(already_done, file="data/clintrials_history_active_studies.rds")
