...
all_results = unzip('Y:/clinicaltrials/AllPublicXML.zip',list=TRUE) %>% data.frame() %>%
  filter(Name!='Contents.txt') %>% mutate(FileName = gsub(".*/","",Name),
                                          FolderName = gsub("/.*","",Name))



#determine number of files per folder for proportional sampling
subfolder_n = all_results %>% group_by(FolderName) %>% summarise(n_files = length(unique(Name)),.groups='drop') %>%
  mutate(prop_sample = n_files/sum(n_files))

total_records_sampled = 100000 #APROX 25% OF TOTAL RECORDS

#choose a random sample of 20% within each subfolder
sample_folder = distinct(all_results,FolderName) %>% pull(FolderName)

for(k in seq_along(sample_folder)){
sample_p = filter(subfolder_n,FolderName==sample_folder[k]) %>% pull(prop_sample)

sample_files = filter(all_results,FolderName==sample_folder[k]) %>% sample_n(ceiling(sample_p*total_records_sampled)) %>% pull(Name)
unzip('Y:/clinicaltrials/AllPublicXML.zip',files=sample_files,exdir = "Y:/clinicaltrials/data")
cat('Processed subfolder ',sample_folder[k],';',length(sample_files),' records sampled','\n',sep='')
}
#not run
